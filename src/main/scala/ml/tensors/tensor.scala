...
sealed trait Tensor[T]:
  ...
  def length: Int = shape.headOption.getOrElse(0)

  def shape(axis: Int): List[Int] = shape.drop(axis)

  def meta[T: ClassTag] = 
    s"shape: ${shape.mkString("x")}, ${getClass.getSimpleName}[${summon[ClassTag[T]]}]"
...
object Tensor3D:
  def apply[T: ClassTag](matrices: Tensor2D[T]*): Tensor3D[T] =
    Tensor3D(matrices.toArray.map(_.data))
